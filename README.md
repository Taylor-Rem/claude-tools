...
| `bin/img` | Pictures and video: `img stock` (Pexels search + download), `img gen` (Gemini image generation, style presets like `--style plateful-menu`), `img edit` (instructed edit of a real photo), `img video` (Veo: `--seconds`, `--aspect`, `--ref` a first frame, `--from` a clip someone sent, `--length N` for a long one stitched from 8s + extensions, `--dry-run` to see the price first), `img describe` (watch a clip and say what's in it), `img credit` / `img balance` (dollars on account against the daily cap — how a chat room pays for more), `img styles`, `img info`, `img doctor`. Every image gets a JSON sidecar recording its provenance, and every generation writes a ledger line. Spending limits come from the environment (`IMG_VIDEO_MAX_SECONDS`, `_MAX_RESOLUTION`, `_MAX_USD`, `_DAILY_CAP`, `_WHO`, …) so a caller like the relay can cap a room without trusting its prompt. |
...
| `bin/meme` | Memes for the chat rooms: `meme fresh [sub ...]` (the newest from a curated subreddit list, keyless through meme-api.com, or Reddit proper with `REDDIT_CLIENT_ID`/`REDDIT_CLIENT_SECRET`), `meme search "<topic>"`, `meme make "<template>" "top" "bottom"` (imgflip templates captioned with ImageMagick + `fonts/Anton-Regular.ttf`, or by imgflip itself with a login), `meme gif` (Tenor key), `meme subs`, `meme templates`, `meme doctor`. Downloads into `generated/`, prints `<path>` per line for SEND-FILE, and skips anything already handed out (`.meme-seen.json`). |
